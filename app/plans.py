...
# Plans are sold as prepaid **payment verifications**.
# In the DB these are stored as `credits`.
# Verifications do not expire.
PLANS: dict[str, Plan] = {
    "trial": Plan(
        id="trial",
        title="Trial",
...
def recommend_purchase(
    additional_verifications_needed: int,
    *,
    max_topups: int = 3,
) -> dict | None:
    """Recommend what to buy to cover `additional_verifications_needed`.
    ...
    Returns a dict:
      {plan_id, quantity, sats_total, verifications_total, verifications_over_need, plan}
    """

    additional_verifications_needed = int(additional_verifications_needed)
    if additional_verifications_needed <= 0:
        return None
    ...
    for plan in PLANS.values():
        if int(plan.verifications) <= 0:
            continue
        q = int(ceil(additional_verifications_needed / float(plan.verifications)))
        sats_total = int(plan.price_sats) * q
        verifications_total = int(plan.verifications) * q
        options.append(
            {
                "plan_id": plan.id,
                "quantity": q,
                ...
                "verifications_over_need": verifications_total - additional_verifications_needed,
                "plan": plan.to_dict(),
            }
        )
    ...
    # Lowest cost; tie-break: fewer topups; then less overshoot
    candidates.sort(key=lambda o: (o["sats_total"], o["quantity"], o["verifications_over_need"]))
